...
import { CreateUserRequest, UpdateUserRequest, User } from '../model/user.schema';
import { toast } from 'sonner';
import { ApiError } from '@/shared/lib/api-error';

// Mock Data
const mockUsers: User[] = [
  { id: 'USR-001', name: 'Admin Utama', email: 'admin@docai.com', role: 'admin', status: 'active', lastLogin: '2026-03-15 10:00' },
  { id: 'USR-002', name: 'Operator Satu', email: 'operator1@docai.com', role: 'operator', status: 'active', lastLogin: '2026-03-14 15:30' },
  { id: 'USR-003', name: 'Operator Dua', email: 'operator2@docai.com', role: 'operator', status: 'inactive', lastLogin: '2026-03-10 09:45' },
];
  ...
  const createMutation = useMutation({
    mutationFn: async (payload: CreateUserRequest) => {
      if (isMock) {
        ...
        return {
          data: { id: `USR-${Math.floor(Math.random() * 1000)}`, ...payload, status: 'active', lastLogin: '-' },
