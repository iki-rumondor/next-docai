          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              ...
              {(job.status === "completed" || job.status === "failed") &&
                (job.processing_duration !== undefined ||
                  job.processing_time?.duration_sec !== undefined) && (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="flex items-center gap-1.5 text-primary cursor-help bg-primary/5 px-2 py-0.5 rounded-md border border-primary/20">
                        ...
                        {formatDuration(
                          job.processing_time?.duration_sec ??
                            job.processing_duration,
                        )}
                      </span>
                    </TooltipTrigger>
                    <TooltipContent>Processing Time Total</TooltipContent>
                  </Tooltip>
                )}
              {job.ai_usage && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span className="flex items-center gap-1.5 text-orange-600 font-bold cursor-help bg-orange-50 px-2 py-0.5 rounded-md border border-orange-200">
                      ...
                      {job.ai_usage.total_price.toFixed(5)}
                    </span>
                  </TooltipTrigger>
                  <TooltipContent>
                    AI Usage Models: {job.ai_usage.model} (Tokens:{" "}
                    {job.ai_usage.total_tokens})
                  </TooltipContent>
                </Tooltip>
              )}
            </div>
            ...
            {isLoading ? (
              <div className="space-y-3">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-24 w-full rounded-2xl bg-muted animate-pulse"
                  />
                ))}
              </div>
            ) : documents.length > 0 ? (
                    ...
                    <div className="flex items-center justify-between px-2 py-2">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          ...
                          disabled={
                            page >= documentsData.pagination.total_pages
                          }
                        >
