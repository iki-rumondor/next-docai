...
import { compareJsonStrict, ComparisonResult } from "@/shared/lib/json-utils";
...
export const ComparisonTool = () => {
  ...
  const sourceFiles = filesResponse?.data || [];
  ...
  const documentsA = useMemo(() => docsResponseA?.data || [], [docsResponseA]);
  const documentsB = useMemo(() => docsResponseB?.data || [], [docsResponseB]);

  // Sync raw data A to textarea (using state-during-render pattern to avoid useEffect cascading renders)
  ...
  const handleCompare = () => {
    ...
    try {
      const comparison = compareJsonStrict(jsonA, jsonB);
      setResult(comparison);
      
      if (comparison.isIdentical) {
        toast.success("Documents are 100% identical!");
      } else {
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-card/30 border border-border/40 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            {(isLoadingDocsA || isLoadingDocsB) && <Loader2 className="h-3 w-3 animate-spin text-muted-foreground" />}
          </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Select value={selectedFileIdA} onValueChange={(val) => {
                    ...
                    <SelectContent>
                      {sourceFiles
                        .filter((file: SourceFile) => file.id !== selectedFileIdB)
                        .map((file: SourceFile) => (
          ...
          <Card className="border-border/40 shadow-sm overflow-hidden bg-card/50 backdrop-blur-sm">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Select value={selectedFileIdB} onValueChange={(val) => {
                    ...
                    <SelectTrigger className="h-8 flex-1 text-[11px] bg-background/50 border-secondary/20">
                      <SelectValue placeholder="1. Select Source File" />
                    </SelectTrigger>
                    <SelectContent>
                      {sourceFiles
                        .filter((file: SourceFile) => file.id !== selectedFileIdA)
                        .map((file: SourceFile) => (
                        <SelectItem key={file.id} value={file.id}>
                          <span className="truncate">{file.file_name}</span>
                        </SelectItem>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isLoadingRawB && <Loader2 className="h-3 w-3 animate-spin text-secondary" />}
                </div>
              </div>

              <div className="flex items-center justify-between pt-1 border-t border-border/20">
                <Badge variant="outline" className="h-5 px-1.5 text-[9px] bg-secondary/5 text-secondary border-secondary/10">INPUT B</Badge>
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="h-6 w-6 text-muted-foreground"
            ...
            <CardContent className="p-0">
              <textarea
                ...
                placeholder="Paste second JSON here or select a document..."
                className="w-full h-[400px] p-4 bg-transparent font-mono text-[13px] leading-relaxed resize-none outline-none focus:ring-0 placeholder:text-muted-foreground/50 text-foreground"
                spellCheck={false}
              />
            </CardContent>
                    ...
                    <tbody className="divide-y divide-border/20 font-mono">
                      {result.diffDetails.filter(d => !d.isMatch).map((diff, idx) => (
                        <tr key={idx} className="hover:bg-accent/20 transition-colors">
                          ...
                          <td className="py-2 px-4 text-primary bg-primary/5">{diff.valueB}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
